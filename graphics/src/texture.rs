...
use self::stb_image::image::{Image, LoadResult};
use std::fmt;
use std::fmt::Debug;
...
impl Debug for Texture2D {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "Texture2D {{ format: {}, handle: {} }}",
            self.format, self.handle
        )
    }
}
...
impl Texture2D {
    pub fn new(path: PathBuf, format: GLenum) -> Texture2D {
        ...
        let image = match image {
            ...
            LoadResult::ImageF32(..) => {
                panic!("Found floating point texture, use Texture2D_HDR instead.")
            }
        ...
        unsafe {
            gl::TextureSubImage2D(
                ...
                0,
                0,
                obj.image.width as i32,
                obj.image.height as i32,
                pixel_formats[obj.image.depth],
                gl::UNSIGNED_BYTE,
                obj.image.data.as_ptr() as *const GLvoid,
            );
    ...
    fn get_mipmap_levels(&self) -> i32 {
        1 + (self.image.width.max(self.image.height) as f32)
            .log2()
            .floor() as i32;
        1
    }
